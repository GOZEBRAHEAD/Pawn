...
v#0.1 = CreaciÛn del mod.
v#0.2 = Mucha mejora del cÛdigo, agregados los efectos al disparar/llegar a 5 kills.
v#0.3 = Agregados mejores efectos al llegar a 5 kills.
v#0.4 = Se optimizÛ todo el mod para que consuma lo menos posible.
v#0.5 = Se agregaron las clases (5 en total).
v#0.6 = Se agregaron las granadas (2 en total) y las trampas.
v#0.7 = Se cambiÛ el tipo de detecciÛn cuando matan y son matados los jugadores.
v#0.8 = Se optimizÛ un poco el mod.
...
v#1.3 = Se agregÛ un HUD personal con los "ATP2-POINTS" para no tener que estar viendo el men˙ de info, a dem·s se agregÛ un men˙ de config. de hud.
v#1.4 = Se reemplazÛ la granada congelante por una explosiva (+ sprite y sonido) la cual te saca la mitad de la vida.
v#1.5 = Se removiÛ el HUD personal ya que se usa el men˙ de INFO. Se agregÛ tambiÈn el daÒo hecho y recibido (total) de cada ronda.
v#1.6 = Se agregÛ que haya poca luz en el mapa y un contador de puntajes (TTs y CTs).
v#1.7 = Se agregÛ que cuando un team llegue a 100 puntos, se cambie de mapa.
v#1.8 = Se agregÛ un men˙ de vote para elegir el siguiente MAPA.
...
new const ALL_MENUS[][] =
{
	//"\r[ATRAPA2] \wELEG√ç UN EQUIPOç",
	"\r[ATRAPA2] \wMEN√ö \yPRINCIPAL",
	"\r[ATRAPA2] \wMEN√ö DE \yESTAD√çSTICAS",
	"\r[ATRAPA2] \wMEN√ö DE \yCLASES^n\r- \wRECUERDA DESBLOQUEAR LAS CLASES \r-",
	"\r[ATRAPA2] \wMEN√ö DE \yDESBLOQUEO DE CLASES^n\r- \wRECUERDA QUE CADA CLASE CUESTA \y125 ATP2-POINTS \r-",
...
public show_menu_game(id)
{
	...
	menu_additem(menu, "\wVER \yESTAD√çSTICAS", "1")
...
public show_menu_estadisticas(id)
{
	...
	"\wTU INFORMACI√ìN:^n\r- \wATP2-POINTS: \y[%d]^n\r- \wDA√ëO HECHO: \y[%d]^n\r- \wDA√ëO RECIBIDO: \y[%d]", 
